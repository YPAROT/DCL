...
int exportCsvDialog::exec(QStringList tableHeaders, QString tableName)
{
    ...
    for (int row = 0; row < tableHeaders.count(); ++row) {
        ...
        int index = tableHeaders.indexOf(
            QWidgetSettings::value(m_tableName + "_toCSV_colIndex_" + QString::number(row), "")
                .toString());
        if (index >= 0)
        ...
        ui->columnsTableWidget->setItem(row,
                                        1,
                                        new QTableWidgetItem(
                                            QWidgetSettings::value(m_tableName + "_toCSV_colName_"
                                                                       + QString::number(row),
                                                                   "")
                                                .toString()));
...
void exportCsvDialog::accept()
{
    for (int row = 0; row < ui->columnsTableWidget->rowCount(); ++row) {
        ...
        if (comboBox)
            QWidgetSettings::setValue(m_tableName + "_toCSV_colIndex_" + QString::number(row),
                                      comboBox->currentText());
        QWidgetSettings::setValue(m_tableName + "_toCSV_colName_" + QString::number(row),
                                  ui->columnsTableWidget->item(row, 1)->text());
    }
