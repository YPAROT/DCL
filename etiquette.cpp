...
Etiquette::Etiquette(QWidget *parent) : QWidget(parent)
{
    my_Lignes.clear();
    ...
    my_printer->setPageSize(QPrinter::A4);
}
...
void Etiquette::paintEvent(QPaintEvent *e)
{
    ...
    painter.drawRoundedRect(center_x-my_Cadre_width/2,center_y-taille_cadre_y/2,my_Cadre_width,taille_cadre_y,5,5);
...
//    qDebug()<<taille_page_mm;
    ...
    double taille_reelle_x = static_cast<double>(my_Cadre_width)/taille_page_px.width()*taille_page_mm.width();
//    qDebug()<<taille_reelle_x;

    double taille_reelle_y = static_cast<double>(taille_cadre_y)/taille_page_px.height()*taille_page_mm.height();
//    qDebug()<<taille_reelle_y;

    QString info_etiquette = "Largeur: "+QString::number(taille_reelle_x)+" mm / Hauteur: "+QString::number(taille_reelle_y)+" mm";
...
void Etiquette::NewParameters(QStringList &Name, QStringList &Value, QVector<int> &Police)
{
    ...
    for(int i=0;i<nb_param;i++)
    {
        ...
        if(TempFontSize.width(TempStr)>my_Cadre_width-20) //10pxl de chaque côté du cadre
        {
            ...
            while(SplitList.size()>0)
            {
                ...
                if(TempFontSize.width(TempStr+" "+SplitList.at(0))<=my_Cadre_width-20)//10pxl de chaque côté du cadre
                {
                    TempStr+=" "+SplitList.takeFirst();
                }
                else
...
//    qDebug() << "Page mm :" << my_printer->pageRect(QPrinter::Millimeter) << my_printer->paperRect(QPrinter::Millimeter);
    ...
    pen.setWidth(4);
    painter.setPen(pen);
    painter.drawRoundedRect(center_x-my_Cadre_width/2,center_y-(sizeText_y+20)/2,my_Cadre_width,sizeText_y+20,5,5);
