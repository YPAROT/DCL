    ...
    , ui(new Ui::MainWindow)
{
    ...
    QObject::connect(this,
                     SIGNAL(SendParamToEtiquette(QStringList &, QStringList &, QVector<int> &)),
                     etiq,
                     SLOT(NewParameters(QStringList &, QStringList &, QVector<int> &)));
    QObject::connect(ui->pushButton_print_etq,
                     SIGNAL(clicked(bool)),
                     etiq,
                     SLOT(PrintSelectionPDF()));

    connect(ui->dclCompleteSqlTableWidget,
            SIGNAL(itemDoubleClicked(const QModelIndex &)),
            this,
            SLOT(dclCompleteSqlTableDoubleClicked(const QModelIndex &)));
    connect(ui->dclPerBoardSqlTableWidget,
            SIGNAL(itemDoubleClicked(const QModelIndex &)),
            this,
            SLOT(dclPerBoardSqlTableDoubleClicked(const QModelIndex &)));
...
void MainWindow::tableViewUpdated()
{
    ...
    if (m_tableViewManager->tablesNames().indexOf(currentText) >= 0)
        ui->customTableComboBox->setCurrentIndex(
            m_tableViewManager->tablesNames().indexOf(currentText));
    ...
    if (!filename.isEmpty()) {
        if ((!QFile::exists(filename))
            || (QMessageBox::information(NULL,
                                         "Nouvelle DCL",
                                         "Le fichier existe deja, la BDD contenue dans le fichier "
                                         "va etre effacée, continuer ?",
                                         QMessageBox::Yes | QMessageBox::No)
                == QMessageBox::Yes)) {
            QSqlError err = m_dclSQLManager->newDCL(filename);
            if (err.type() != QSqlError::NoError)
                QMessageBox::warning(this,
                                     tr("Unable to create database"),
...
void MainWindow::refreshDCLAll()
{
    ...
    //passage Qt5 à Qt6
    QRegularExpression fromRx("FROM\\s+Composants\\s+(\\w+)");
    QRegularExpressionMatch match = fromRx.match(queryStr);
    if (match.hasMatch()) {
        queryStr.replace(QRegularExpression("^select\\s+", QRegularExpression::CaseInsensitiveOption), "select " + match.captured(1) + ".ID,");
    }
    else
    {
        queryStr.replace(QRegularExpression("^select\\s+", QRegularExpression::CaseInsensitiveOption), "select Composants.ID,");
    }
    // if (fromRx.indexIn(queryStr) >= 0) {
    //     queryStr.replace(QRegularExpression("^select\\s+", Qt::CaseInsensitive),
    //                      "select " + fromRx.cap(1) + ".ID,");
    // } else
    //     queryStr.replace(QRegularExpression("^select\\s+", Qt::CaseInsensitive), "select Composants.ID,");
    ...
    QStringList result = m_dclSQLManager->execQueryAndGetResult(
        "SELECT Composants.Part_Number FROM Composants");
...
{
    ui->manualSqlTableWidget->execQuery(ui->sqlEdit->toPlainText(),
...
void MainWindow::on_comboBox_carte_currentIndexChanged(int index)
{
    ...
    ui->comboBox_Model->addItems(
        m_dclSQLManager->getModelForBoard(ui->comboBox_carte->currentText()));
...
void MainWindow::on_comboBox_Model_currentIndexChanged(int index)
{
    ...
    ui->comboBox_Serial->addItems(
        m_dclSQLManager->getSerialForBoard(ui->comboBox_carte->currentText(),
                                           ui->comboBox_Model->currentText()));
}
...
void MainWindow::on_comboBox_Serial_currentIndexChanged(int index)
{
    ...
    queryStr.replace(QRegularExpression("^select\\s+", QRegularExpression::CaseInsensitiveOption), "select DCL.ID,");

    QStringList results = m_dclSQLManager->getListFromQuery(
        "SELECT COUNT(*)"
        " FROM DCL"
        " WHERE DCL.Board_ID=(SELECT Board_ID FROM Board WHERE Board_Name=\""
        + ui->comboBox_carte->currentText() + "\"" + " AND Model=\""
        + ui->comboBox_Model->currentText() + "\"" + " AND Board_serial=\""
...
{
    ui->editBoardSqlTableWidget
        ->showTable("Board",
                    m_dclSQLManager->currentDatabase(),
                    "INSERT INTO Board (Board_Name) VALUES (\"enter board name\");");
    ui->editContactSqlTableWidget->showTable(
        "Procurement Company Contact",
        m_dclSQLManager->currentDatabase(),
        "INSERT INTO \"Procurement Company Contact\" (Name,First_name,Proc_company) VALUES "
        "(\"enter name\",\"first name\",(SELECT Min(Proc_ID) from \"Procurement Company\"));");
    ui->editCompanySqlTableWidget
        ->showTable("Procurement Company",
                    m_dclSQLManager->currentDatabase(),
                    "INSERT INTO \"Procurement Company\" (Name) VALUES (\"enter company name\");");
    ui->editComponentsSqlTableWidget
        ->showTable("Composants",
                    m_dclSQLManager->currentDatabase(),
                    "INSERT INTO \"Composants\" (Family, Part_Number, Status_ID) VALUES (\"???\", "
                    "\"???\", (SELECT Min(ID) from Status));");
    ui->editManufacturerSqlTableWidget
        ->showTable("Manufacturer",
                    m_dclSQLManager->currentDatabase(),
                    "INSERT INTO \"Manufacturer\" (Name) VALUES (\"enter manufacturer name\");");
    ui->editStockageSqlTableWidget
        ->showTable("Stockage",
                    m_dclSQLManager->currentDatabase(),
                    "INSERT INTO \"Stockage\" (Location) VALUES (\"enter new location\");");
    ui->statusSQLTableWidget
        ->showTable("Status",
                    m_dclSQLManager->currentDatabase(),
                    "INSERT INTO \"Status\" (Status) VALUES (\"enter new status\");");
}
...
{
    QString tableName;
    QList<SQLTableForm *> sqlTableForm = ui->editTabWidget->currentWidget()
                                             ->findChildren<SQLTableForm *>();
    ...
    if (!filename.isEmpty()) {
        ...
        if (data.isEmpty())
            QMessageBox::warning(
                this,
                "Fichier CSV",
                "Le fichier CSV est vide ou le séparateur n'est pas \",\" ou \";\" ....");
        else {
            ...
            if (!tableFields.isEmpty()) {
                ...
                if (ui->editTabWidget->currentIndex() == 3) {
                    ...
                    procurementCompanyList = m_dclSQLManager->getListFromQuery(
                        "select Name from \"Procurement Company\"");
                    tableFields.removeOne("Proc_company");
                    if (procurementCompanyList.isEmpty()) {
                        QMessageBox::warning(
                            this,
                            "Import",
                            "Aucun fournisseur n'est défini, ajouter des fournisseurs dans "
                            "l'onglet édition avant d'importer des composants");
                        return;
                    }
                    ...
                    if (statusList.isEmpty()) {
                        QMessageBox::warning(
                            this,
                            "Import",
                            "Aucuns statuts ne sont définis, ajouter des statut dans l'onglet "
                            "édition avant d'importer des composants");
                        return;
                    }
                    procurementCompanyList = m_dclSQLManager->getListFromQuery(
                        "select Name from \"Procurement Company\"");
                    if (procurementCompanyList.isEmpty()) {
                        QMessageBox::warning(
                            this,
                            "Import",
                            "Aucuns fournisseurs ne sont définis, ajouter des fournisseurs dans "
                            "l'onglet édition avant d'importer des composants");
                        return;
                    }
                    tableFields.removeFirst(); // on vire le premier qui est l'ID
                ...
                if (importComponentsDialog->exec(tableName,
                                                 tableFields,
                                                 data.first(),
                                                 statusList,
                                                 procurementCompanyList)
                    == QDialog::Accepted) {
                    ...
                    if (ui->editTabWidget->currentIndex() == 0) {
                        QStringList statusId = m_dclSQLManager->getListFromQuery(
                            "select ID from status where Status=\""
                            + importComponentsDialog->status() + "\";");
                        startQuery = startQuery + statusId.first() + ",";
                        QStringList procurementCompanyId = m_dclSQLManager->getListFromQuery(
                            "select Proc_Id from \"Procurement Company\" where Name=\""
                            + importComponentsDialog->fournisseur() + "\";");
...
{
    // QStringList topo = ranges.split(",", Qt::SkipEmptyParts);
    // if (!topo.isEmpty()) {
    ...
    if (!topo.isEmpty()) {
        for (int topoIndex = topo.count() - 1; topoIndex >= 0; topoIndex--) {
            if (topo.at(topoIndex).contains("..")) {
                QStringList items = topo.at(topoIndex).split("..");
                QRegularExpression rx("(\\S*\\D)(\\d+)$");
                QRegularExpressionMatch matchFirst = rx.match(items.first());
                QRegularExpressionMatch matchLast = rx.match(items.last());
                ...
                if (matchFirst.hasMatch()) {
                    firstIndex = matchFirst.captured(2).toInt();
                    firstRefStr = matchFirst.captured(1);
                    if (matchLast.hasMatch()) {
                        lastIndex = matchLast.captured(2).toInt();
                        lastRefStr = matchLast.captured(1);
                    }
                }
                ...
                if ((lastIndex >= 0) && (firstIndex >= 0) && (firstRefStr == lastRefStr)) {
                    topo.removeAt(topoIndex);
                    for (int i = qMax(firstIndex, lastIndex); i >= qMin(firstIndex, lastIndex); i--)
                        topo.insert(topoIndex, firstRefStr + QString::number(i));
                } else {
                    return "";
                }
            }
...
void MainWindow::on_editionAddPushButton_clicked()
{
    if (!ui->editionTopoLineEdit->text().isEmpty()) {
        ...
        if (topo.isEmpty()) {
            QMessageBox::warning(this,
                                 "Saisie repères topo",
                                 "Erreur, mauvaise saisie : " + ui->editionTopoLineEdit->text());
            return;
        }
        ...
        if (!componentList.isEmpty()) {
            ...
            if (topo.contains(","))
                ui->editionLogTextEdit->append("Ajouter les refs \"" + topo + "\" de type "
                                               + componentList.first() + serial);
            else
                ui->editionLogTextEdit->append("Ajouter la ref \"" + topo + "\" de type "
                                               + componentList.first() + serial);
        } else
        ...
        QMessageBox::warning(this,
                             "Saisie reperes topo",
                             "Erreur, pas de reperes topologiques saisis...");
}
...
void MainWindow::on_comboBox_carte_2_currentIndexChanged(int index)
{
    ...
    ui->comboBox_Model_2->addItems(
        m_dclSQLManager->getModelForBoard(ui->comboBox_carte_2->currentText()));
}
...
void MainWindow::on_comboBox_Model_2_currentIndexChanged(int index)
{
    ...
    ui->comboBox_Serial_2->addItems(
        m_dclSQLManager->getSerialForBoard(ui->comboBox_carte_2->currentText(),
                                           ui->comboBox_Model_2->currentText()));
}

void MainWindow::on_comboBox_Serial_2_currentIndexChanged(int index)
{
    Q_UNUSED(index)
    ui->dclEditionSqlTableWidget
        ->execQuery("SELECT Composants.Part_Number,"
                    "Composants.Family,"
...
void MainWindow::on_editionValidateColumnsPushButton_clicked()
{
    if (m_csvData.isEmpty())
        QMessageBox::warning(this,
                             "Fichier CSV",
                             "Pas de fichier csv selectionné ou fichier CSV vide....");
    else if ((ui->editionRefTopoComboBox->currentText()
              != ui->editionRefCompoComboBox->currentText())
             && (ui->editionSerialCompoComboBox->currentText()
                 != ui->editionRefTopoComboBox->currentText())) {
        ...
        QMessageBox::warning(this,
                             ...
                             "Choisir des colonnes differentes dans le CSV....");
}
...
void MainWindow::on_editionEndedPushButton_clicked()
{
    ...
    if (!entries.isEmpty()) {
        QStringList boardIds = m_dclSQLManager->getListFromQuery(
            "SELECT Board_ID from Board WHERE Board_Name=\"" + ui->comboBox_carte_2->currentText()
            + "\"" + " AND Model=\"" + ui->comboBox_Model_2->currentText() + "\""
            + " AND Board_serial=\"" + ui->comboBox_Serial_2->currentText() + "\";");
        if (boardIds.isEmpty()) {
            ui->editionSQLLogTextEdit->append("Erreur: Pas de carte selectionné ou impossible de "
                                              "retrouver la carte dans la base de données");
            return;
        }
        ...
        //     QRegularExpression rx("\\\"(\\S*)\\\"\\s*de\\s+type\\s+(\\S+)");
        //     QRegularExpression serialRx("\\((.+)\\)\\s*$");
        //     if (rx.indexIn(entries.at(i)) >= 0) {
        //         QStringList topos = rx.cap(1).split(",");
        //         QStringList IDs = m_dclSQLManager->getListFromQuery(
        //             "SELECT ID FROM Composants where Part_Number=\"" + rx.cap(2) + "\";");
        //         if (IDs.isEmpty())
        //             ui->editionSQLLogTextEdit->append(entries.at(i) + " => NOK, " + rx.cap(2)
        //                                               + " n'existe pas");
        //         else {
        ...
        //                                                       + topos.at(j) + " existe deja");
        //                 else {
        //                     QString queryStr;
        //                     if (serialRx.indexIn(entries.at(i)) < 0)
        //                         queryStr
        //                             = "INSERT into DCL (Composant_ID, Board_ID, Ref_Topo) VALUES ("
        //                               + ID + "," + boardId + ",\"" + topos.at(j) + "\");";
        //                     else
        //                         queryStr = "INSERT into DCL (Composant_ID, Board_ID, Ref_Topo, "
        ...
        //                         ui->editionSQLLogTextEdit->append(entries.at(i) + " => OK pour "
        //                                                           + topos.at(j));
        //                     } else
        //                         ui->editionSQLLogTextEdit->append(
        //                             entries.at(i) + " => NOK, erreur mySql lors de l'insertion pour "
        //                             + topos.at(j) + " (" + m_dclSQLManager->lastError() + ")");
        //                 }
        //             }
        //         }
        //     } else {
        ...
        for (int i = 0; i < entries.count(); ++i) {
            ...
            QRegularExpressionMatch rxMatch = rx.match(entries.at(i));
            if (rxMatch.hasMatch()) {
                QStringList topos = rxMatch.captured(1).split(",");
                QStringList IDs = m_dclSQLManager->getListFromQuery(
                    "SELECT ID FROM Composants where Part_Number=\"" + rxMatch.captured(2) + "\";");

                if (IDs.isEmpty()) {
                    ui->editionSQLLogTextEdit->append(entries.at(i) + " => NOK, " + rxMatch.captured(2) + " n'existe pas");
                } else {
                    QString ID = IDs.first();
                    for (int j = 0; j < topos.count(); ++j) {
                        if (refTopoList.indexOf(topos.at(j)) >= 0) {
                            ui->editionSQLLogTextEdit->append(entries.at(i) + " => NOK, " + topos.at(j) + " existe déjà");
                        } else {
                            QString queryStr;
                            QRegularExpressionMatch serialRxMatch = serialRx.match(entries.at(i));
                            if (!serialRxMatch.hasMatch()) {
                                queryStr = "INSERT into DCL (Composant_ID, Board_ID, Ref_Topo) VALUES ("
                                           + ID + "," + boardId + ",\"" + topos.at(j) + "\");";
                            } else {
                                queryStr = "INSERT into DCL (Composant_ID, Board_ID, Ref_Topo, Serial_Number) VALUES ("
                                           + ID + "," + boardId + ",\"" + topos.at(j) + "\",\""
                                           + serialRxMatch.captured(1) + "\");";
                            }
...
{
    QStringList ParamNameList = {"Projet",
                                 "Désignation",
                                 "Fabricant",
                                 "Fournisseur",
                                 "Valeur",
    ...
    if (ui->tableWidget_Etiq->selectedItems().size() == 0)
        return;
    QString PN_str = ui->tableWidget_Etiq->selectedItems()
                         .at(0)
                         ->text(); //même si il y en a plusieurs on selectionne le premier seulement
    ...
    QString queryStr = "SELECT Composants.Part_Number,"
                       ...
                       " FROM Composants"
                       " INNER JOIN Manufacturer on Composants.Manuf_ID=Manufacturer.Manuf_ID"
                       " INNER JOIN \"Procurement Company\" ON \"Procurement "
                       "Company\".Proc_ID=Composants.Procurement_Company"
                       " INNER JOIN Stockage on Composants.Stock_location=Stockage.ID"
                       " WHERE Composants.Part_Number=\""
                       + PN_str + "\";";
    QStringList result = m_dclSQLManager->execQueryAndGetResult(queryStr);
    if (!result.isEmpty()) {
        QStringList values = result.first().split(";", Qt::KeepEmptyParts);
...
void MainWindow::on_pushButton_etq_all_clicked()
{
    ...
    QRectF taille_page_mm = pageLayout.paintRect(QPageLayout::Millimeter);
    QRectF taille_page_px_f = printer.pageRect(QPrinter::DevicePixel);
    ...
    int hauteur_etiquette_pxl = hauteur_etiquette / taille_page_mm.height()
                                * taille_page_px.height();
    ...
    for (int page = 0; page < nb_pages; page++) {
        ...
        for (index_etiquette = 0; index_etiquette < 12; index_etiquette++) {
            ...
            painter.drawRoundedRect(x_coord,
                                    y_coord,
                                    largeur_etiquette_pxl,
                                    hauteur_etiquette_pxl,
                                    5,
                                    5);

            //Récupération des données étiquettes
            //qDebug()<<"index etiquette: "<<page*12+index_etiquette;
            donnees_etiquette = GetEtiquetteData(
                ui->tableWidget_Etiq->item(page * 12 + index_etiquette, 0)->text());
            ...
            for (int i = 0; i < donnees_etiquette.ParamPoliceList.size(); i++) {
                rapport_font << static_cast<double>(donnees_etiquette.ParamPoliceList.at(i))
                                    / donnees_etiquette.ParamPoliceList.at(0);
            }
            ...
            while (!fit_ok) {
                ...
                for (int j = 0; j < nb_ligne_etiquette; j++) {
                    ...
                    TempStr = donnees_etiquette.ParamNameList.at(j) + ": "
                              + donnees_etiquette.ParamValueList.at(j);

                    if (TempFontSize.horizontalAdvance(TempStr)
                        > largeur_etiquette_pxl - 20) //10pxl de chaque côté du cadre
                    {
                        //Split en fonction des espaces
                        SplitList = TempStr.split(QRegularExpression("\\s+")); //blancs enlevés
...
Etiquette_Data MainWindow::GetEtiquetteData(QString PN_str)
{
    ...
    resultat.ParamNameList << "Projet" << "Désignation" << "Fabricant" << "Fournisseur" << "Valeur"
                           << "Tolérance" << "Max. rating" << "Boîtier" << "Date Code"
                           << "Qualifié spatial" << "ITAR/EAS" << "Relifed" << "Quantité commandée"
                           << "Lieu de stockage";
    QString queryStr;
...
void MainWindow::dclPerBoardSqlTableDoubleClicked(const QModelIndex &index)
{
    ...
    t->setColumnCount(1);
    t->setRowCount(24);
    t->setVerticalHeaderLabels({"Désignation",
                                "Famille",
                                "Group Code",
                                "Description",
                                "Valeur",
    ...
    dclID = ui->dclPerBoardSqlTableWidget->sqlTableView()
                ->model()
                ->data(ui->dclPerBoardSqlTableWidget->sqlTableView()->model()->index(index.row(), 0))
                .toString();
    QString queryStr = "SELECT Composants.Part_Number,"
                       " Composants.Family,"
                       " Composants.Group_code,"
                       ...
                       " WHERE"
                       " Composants.ID=(SELECT Composant_ID FROM DCL WHERE ID=\""
                       + dclID + "\");";
...
void MainWindow::dclCompleteSqlTableDoubleClicked(const QModelIndex &index)
{
    ...
    //Récupération de tous les éléments!!!
    composantID = ui->dclCompleteSqlTableWidget->sqlTableView()
                      ->model()
                      ->data(
                          ui->dclCompleteSqlTableWidget->sqlTableView()->model()->index(index.row(),
                                                                                        0))
                      .toString();
    QString queryStr = "SELECT Composants.Part_Number,"
                       " Composants.Family,"
...
void MainWindow::on_customTableComboBox_currentIndexChanged(int index)
{
    ...
    if (index == 0)
        ...
        QString queryStr = m_tableViewManager
                               ->requestForTableIndex(ui->customTableComboBox->currentIndex() - 1)
                               .trimmed();
        queryStr.replace("$BOARD_NAME", ui->comboBox_carte->currentText());
        queryStr.replace("$BOARD_MODEL", ui->comboBox_Model->currentText());
        queryStr.replace("$BOARD_SERIAL", ui->comboBox_Serial->currentText());
...
{
    ui->customSqlTableWidget->execQuery(m_tableViewManager->requestForTableIndex(
                                            ui->customTableComboBox->currentIndex() - 1),
                                        m_dclSQLManager->currentDatabase());
}
            ...
            if (!filename.isEmpty()) {
                ...
                for (int row = 0; row < sqlTable->model()->rowCount(); ++row) {
                    ...
                    for (int column = 0; column < columnIndexes.count(); ++column)
                        strList << sqlTable->model()
                                       ->index(row, columnIndexes.at(column))
                                       .data()
                                       .toString();
                ...
                if (QtCSV::Writer::write(filename, strData, ";") == false)
                    QMessageBox::warning(this,
                                         ...
                                         "Erreur lors de l'enregistrement du csv...");
            }
        }
    }
}
...
{
    QStringList boardIds = m_dclSQLManager->getListFromQuery(
        "SELECT Board_ID from Board WHERE Board_Name=\"Trash " + ui->comboBox_carte->currentText()
        + "\"" + " AND Model=\"" + ui->comboBox_Model->currentText() + "\"" + " AND Board_serial=\""
        + ui->comboBox_Serial->currentText() + "\";");
    if (boardIds.isEmpty()) {
        qDebug() << "creation de la trash";
        m_dclSQLManager->execQuery(
            "INSERT INTO Board (Board_Name,Model,Board_serial) VALUES (\"Trash "
            + ui->comboBox_carte->currentText() + "\", \"" + ui->comboBox_Model->currentText()
            + "\", \"" + ui->comboBox_Serial->currentText() + "\");");
    ...
    boardIds = m_dclSQLManager->getListFromQuery(
        "SELECT Board_ID from Board WHERE Board_Name=\"Trash " + ui->comboBox_carte->currentText()
        + "\"" + " AND Model=\"" + ui->comboBox_Model->currentText() + "\"" + " AND Board_serial=\""
        + ui->comboBox_Serial->currentText() + "\";");
    ...
    if (!boardIds.isEmpty()) {
        QModelIndexList currentSelection
            = ui->dclPerBoardSqlTableWidget->sqlTableView()->selectionModel()->selectedIndexes();
        for (int i = 0; i < currentSelection.count(); ++i) {
            if (currentSelection.at(i).column() != 0)
                continue;

            m_dclSQLManager->execQuery("UPDATE DCL SET Board_ID =\"" + boardIds.first()
                                       + "\" WHERE ID=\"" + currentSelection.at(i).data().toString()
                                       + "\";");
        }
    }
