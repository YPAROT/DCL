    ...
    , ui(new Ui::EditTableViewForm)
{
    ...
    m_deleteRowAction = new QAction(tr("Supprimer la ligne selectionnée..."),
                                    ui->editTableViewTableWidget);
...
void EditTableViewForm::on_editTableViewTableWidget_cellDoubleClicked(int row, int column)
{
    ...
    if (row == 1)
        infoLabel = "Utiliser $BOARD_NAME, $BOARD_MODEL et $BOARD_VERSION dans la requete pour "
                    "prendre en compte le choix de carte par l'interface graphique.";

    if (editDialog.execute(m_tableViewManager->nameForTableIndex(row),
                           m_tableViewManager->requestForTableIndex(row),
                           (row > 1),
                           infoLabel,
                           groupByRequest)) {
        m_tableViewManager->setNameAndRequestForTableIndex(row,
                                                           editDialog.tableName(),
                                                           editDialog.request());
    ...
    if (!selectedItems.isEmpty()) {
        ...
        if (row > 1) {
            ...
            QMessageBox::information(this,
                                     ...
                                     "La table " + ui->editTableViewTableWidget->item(row, 0)->text()
                                         + " ne peut pas etre supprimée.");
        }
    }
    emit tableViewUpdated();
}
