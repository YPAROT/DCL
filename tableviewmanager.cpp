...
QString TableViewManager::defaultRequestForBoardDCLTable()
{
    return "SELECT DCL.Ref_Topo,"
           ...
           "##SELECT Composants.Part_Number, GROUP_CONCAT(DCL.Ref_Topo) AS \"Ref_Topo\", "
           "DCL.Serial_Number,Composants.Family,Composants.Group_code,Composants.Description,"
           "Composants.Value,Composants.Tolerance,Composants.Maximum_rating,Composants.Package,"
           "Composants.Quality_level,Composants.Date_Code,Composants.Relifed,Composants.ITAR_EAS,"
           "Manufacturer.Name FROM DCL INNER JOIN Composants ON Composants.ID=DCL.Composant_ID "
           "INNER JOIN Manufacturer ON Composants.Manuf_ID=Manufacturer.Manuf_ID WHERE "
           "DCL.Board_ID=(SELECT Board_ID FROM Board WHERE Board_Name=\"$BOARD_NAME\" AND "
           "Model=\"$BOARD_MODEL\" AND Board_serial=\"$BOARD_SERIAL\") GROUP BY "
           "Composants.Part_Number;";
}
...
bool TableViewManager::setNameAndRequestForTableIndex(int tableIndex,
...
bool TableViewManager::loadConfFromFile(QString filename)
{
    if (QFile::exists(filename)) {
        ...
        m_customTablesList.append(
            QStringList() << "DCL complète"
                          << settings.value("fullDCLRequest", defaultRequestForFullDCLTable())
                                 .toString());
        m_customTablesList.append(
            QStringList() << "DCL par carte"
                          << settings.value("boardDCLRequest", defaultRequestForBoardDCLTable())
                                 .toString());
